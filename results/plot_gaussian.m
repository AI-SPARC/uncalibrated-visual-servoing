...
data = readtable("data/gaussian_kf/results.csv");
...
for i = 1:size(rhos)
    ...
    for j = 1:size(experiments)
        ...
        error1 = rmmissing(error1);
        error2 = rmmissing(error2);
        error3 = rmmissing(error3);
        error4 = rmmissing(error4);
        error5 = rmmissing(error5);
        error6 = rmmissing(error6);
        error7 = rmmissing(error7);
        error8 = rmmissing(error8);
        ...
        % ITAE
        itae = zeros(8, 1);
        itae(1) = experiment.t(idx_e1)' * abs(error1);
        itae(2) = experiment.t(idx_e2)' * abs(error2);
        itae(3) = experiment.t(idx_e3)' * abs(error3);
        itae(4) = experiment.t(idx_e4)' * abs(error4);
        itae(5) = experiment.t(idx_e5)' * abs(error5);
        itae(6) = experiment.t(idx_e6)' * abs(error6);
        itae(7) = experiment.t(idx_e7)' * abs(error7);
        itae(8) = experiment.t(idx_e8)' * abs(error8);
